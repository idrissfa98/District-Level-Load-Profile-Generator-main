...
# Reading High_Level_Dwellings.csv 
def read_high_level_dwellings():    
    ...
    ToUser=pd.to_datetime(high_level_dwellings.iloc[:,2])
...
# Read load profile "High_Level_ED_NonResidential.csv" 
...
# Reading Low_Level_Attributes.csv file (specification of recorded houses)
def read_DBattributes():
    Attributesdata=pd.read_csv ("Low_Level_Attributes.csv", na_values=["nan", "NaN", "NAN", "na", "n/a", "/", "-", "--", ".", "_", "o", "O","None"],encoding='latin1')
...
# Reading recorded profiles of houses (Electricity Demand or ED)
def read_DBed ():   
    ed=pd.read_csv ("Low_Level_ED.csv",na_values=["nan", "NaN", "NAN", "na", "n/a", "/", "-", "--", ".", "_", "o", "O","None"],encoding='latin1')
    return ed   
 
# Fill missed data in ED (Low_Level_ED.csv) by value in last week
...
# Find k-nearest neighbors and return corresponding profiles
...
# Compute barycenter on the KNNs via DTW for each day
def DTW_Averaging (KNN_EDs,From_edited,To_edited):
    ...
    if  From_edited.year==To_edited.year:      
        ...
        Range_ed=pd.date_range(start=From_edited.strftime("%Y-%m-%d")+ " 00:00", end=To_edited.strftime("%Y-%m-%d")+ " 23:45", freq="15 min")
        for d in range(len(From_ed)//96, (len(From_ed)//96)+(len(Range_ed)//96), 1):
            ed_day=ed_knn.iloc[d*96:96+(96*d),:]
            ed_day_t=ed_day.T
        ...
        ed=[]
        Range_ed=pd.date_range(start=str(To_edited.year)+ "-01-01 00:00", end=To_edited.strftime("%Y-%m-%d")+ " 23:45", freq="15 min")
...
# Aggregation the generated ED according to the Number of Dwellings
def aggregation_ED (x,DwellingsAttributes,NoDwellings,From,To,ToUser):
    ...
    idx=pd.date_range(start=From.strftime("%Y-%m-%d")+ " 00:00", end=To.strftime("%Y-%m-%d")+ " 23:45", freq="15 min")
    x.index=idx   
    x=x[From:ToUser]
    x=x.iloc[:,0].squeeze()
    return x

# loop for generating ED profile of all Dwelling 
...
# Edit "High_Level_ED_NonResidential.csv" according to user-defined time frame
...
# Selecting Non-Residential ED profile
...
# Output file for generated High_Level ED profiles
...
def main():
    ...
    # Read Electricity Demand profiles EDs from Low_Level_ED.csv
    ed=read_DBed()
    # Routine for filling missing values in ED profiles
    ed=fill_Missed_Data(ed)
    # Read Attributes from DB Low_Level_Attributes.csv
    Attributesdata=read_DBattributes()
    # Find value of k  
    # Rule of thumb: according to the literature, k = Sqrt(number of load profiles)
